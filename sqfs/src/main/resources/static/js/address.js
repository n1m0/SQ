...
$.extend({
	initialize:function(readJson){
		if(readJson.selectIds==undefined || readJson.selectIds=="" || readJson.selectIds[0]==undefined || readJson.selectIds[0]=="" || readJson.selectIds[1]==undefined || readJson.selectIds[1]=="" || readJson.selectIds[2]==undefined || readJson.selectIds[2]==""){
			alert("请填写省、市、区对应的下拉框的id");
			return ;
		}
		s = readJson.selectIds;
		opt0 = readJson.opt0;
		if(opt0==undefined || opt0=="" || opt0.length!=3){
		    opt0 = ["请选择省", "请选择市", "请选择区/县"];
		}
		setup();
		if(readJson.province!=undefined && readJson.province!=""){
			preselect(readJson.province);
			if(readJson.city!=undefined && readJson.city!=""){
			    $("#"+readJson.selectIds[1]+" option[value='"+readJson.city+"']").attr("selected",true);
				change(2);
				if(readJson.district!=undefined && readJson.district!=""){
					$("#"+readJson.selectIds[2]+" option[value='"+readJson.district+"']").attr("selected",true);
				}
			}
		}
